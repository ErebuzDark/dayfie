...
export default function CommentCard({ comment, postId, replies = [], isReply = false }) {
  ...
  const displayName =
    (user?.uid === comment.authorId ? userProfile?.displayName : authorProfile?.displayName) ||
    comment.authorName ||
    "Anonymous";
  const authorPhoto =
    (user?.uid === comment.authorId ? userProfile?.photoURL : authorProfile?.photoURL) ||
    ...
    <div className={`flex gap-3 ${isReply ? "py-1 ml-9" : "py-2"}`}>
      ...
      <div className="flex-1">
        ...
        <div className="mt-2">
          <ReactionBar
            item={comment}
            onToggle={(commentId, uid, reactionKey) => toggleCommentReaction(postId, commentId, uid, reactionKey)}
          />
        </div>
        {replies?.length > 0 && (
          <div className="mt-3">
    ...
    <div>
      ...
      {replies.length > VISIBLE && (
        <button
          ...
          style={{ background: "none", border: "none", color: "oklch(58% 0 0)", cursor: "pointer", marginTop: 6 }}
        >
